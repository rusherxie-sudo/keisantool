...
describe('incomeFromSalary（年収→給与所得の概算・令和7年改正後）', () => {
  // 令和7年度税制改正: 給与所得控除の最低保障額 55万→65万、適用範囲を年収190万まで拡大。
  // 190万超の区間は従来どおり。令和8年度国保は令和7年中の所得が基礎のため新表を使う。
  // 速算表（令和7年分以降）:
  //   〜190万      : 控除65万（最低保障）
  ...
  it('給与収入100万円 → 所得35万円（最低控除65万）', () => {
    expect(incomeFromSalary(1000000)).toBe(350000);
  });

  it('給与収入65万ちょうど → 所得0（控除65万の境界）', () => {
    expect(incomeFromSalary(650000)).toBe(0);
  });

  it('給与収入64万 → 所得0', () => {
    expect(incomeFromSalary(640000)).toBe(0);
  });

  it('給与収入190万ちょうど → 所得125万円（190万−65万・境界で連続）', () => {
    expect(incomeFromSalary(1900000)).toBe(1250000);
  });

  it('給与収入200万円 → 所得132万円（200万×0.7−8万＝収入×30%+8万控除）', () => {
    expect(incomeFromSalary(2000000)).toBe(1320000);
  });

  it('給与収入400万円 → 所得276万円（控除＝収入×20%+44万）', () => {
    expect(incomeFromSalary(4000000)).toBe(2760000);
  });

  it('給与収入700万円 → 所得520万円（控除＝収入×10%+110万）', () => {
    expect(incomeFromSalary(7000000)).toBe(5200000);
  });

  it('給与収入900万円 → 所得705万円（控除上限195万）', () => {
    expect(incomeFromSalary(9000000)).toBe(7050000);
  });
