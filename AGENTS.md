...
- ⚠️ **改任何页面/数据文件后、部署前，必须刷新 `src/data/lastmod.json` 并一并提交**：CF Pages 构建是 shallow clone（无 git 历史），sitemap lastmod 与页面 JSON-LD dateModified 靠这份提交式 manifest 才能拿到真实提交日，否则会失真为「部署日」。正确顺序是**两步提交**：① 先 `git commit` 页面/数据改动；② 再跑 `node scripts/update-lastmod.mjs`（此时脚本才能读到页面最新的 commit 日期）并 `git commit` 生成的 manifest；③ 一次 `git push` 两个 commit。
